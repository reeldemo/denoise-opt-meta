...
| T1 | **W0** Scaffold `paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v8/` from v7; CHANGELOG / `paper/main.tex` pointer / TITLES; `REVIEW_RESPONSE.md` skeleton | T0 | `denoise-opt-meta/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v8/` exists with copied manuscript; pointer + CHANGELOG updated; REVIEW_RESPONSE skeleton present. Supports AC-5.1 (scaffold), AC-5.2 (skeleton). |
| T2 | **W3** Math + captions: formal $\Theta$, ideal sibling $G$, $\max R$; Fig.1/no-bake captions; Q1–Q2 rows | T1 | AC-2.1, AC-2.2, AC-2.3 checkable in `paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v8/` + REVIEW_RESPONSE. |
...
| T4 | **W2** Vibrato spectrogram script+figure; hear panel + WAV citation; compact WT gallery from existing exports; transfer appendix | T1 | AC-3.1, AC-3.2, AC-3.3, AC-3.4. Artifacts under `reelsynth/brand/artifacts/vibrato_spectrogram/` + figures in `paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v8/figures/`; no new NAS; no wipe of `meta_approach_compare/`. |
...
| T6 | **W5** Results/Discussion/Limitations; rebuild `paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v8/main.pdf`; complete REVIEW_RESPONSE; mirror README → v8; commit+push both repos | T2, T3, T4, T5 | AC-5.1, AC-5.2, AC-5.3. No formal A/B listening claim; no deep-SOTA transfer claim. |
