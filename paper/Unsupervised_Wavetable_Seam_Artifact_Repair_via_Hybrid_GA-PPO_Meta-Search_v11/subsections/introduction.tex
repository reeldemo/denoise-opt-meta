...
\begin{itemize}
  ...
  \item \textbf{Repair operator} (bake operator $\Theta$, short: operator): the searchable local edit applied at the seam (legacy code name: bake cell).
  \item \textbf{Residual operator}: optional tiny learned network inside a repair operator.
  \item \textbf{FitCell}: inner continuous-parameter fit of one candidate $\Theta$ under the residual loss (Appendix~\ref{app:algorithms}).
  \item \textbf{$R$} / \textbf{$R_{\mathrm{blend}}$}: unsupervised seam-quality scores. Whole-curve prolonged $R$ is for debug. $R_{\mathrm{blend}}$ is for primary selection. Details in Section~\ref{sec:residual-score}.
...
They are mechanistic stress tests of that geometry, not claims that an audio-trained bake ``transfers'' as bearing or ECG diagnosis SOTA.
