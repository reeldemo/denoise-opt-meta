...
Figure~\ref{fig:search-learning-curve} plots champion score versus outer-loop evaluation index.
Left: matched $5{,}000$-evaluation hybrid vs Random NAS under \emph{superseded} whole-curve prolonged $R$
(seed \texttt{1902771841}, Dual Cosine ${\approx}0.817$).
Hybrid and Random both clear Dual Cosine within the first few evaluations on that older metric.
Right: v10.1 hybrid under locked $R_{\mathrm{blend}}$ / $J$ ($1{,}200$ evaluations).
Raw champion $R_{\mathrm{blend}}$ clears Dual Cosine immediately and plateaus below the Noise2Noise gate (${\approx}0.975$).
A Random NAS curve under $R_{\mathrm{blend}}$ is not in the released logs (Deferred D1 / \texttt{METRIC\_REGEN.md}).
Plateau boredom in the hybrid loop retunes branch mix when champions stall (Section~\ref{sec:hybrid}). One factual note: under near-ceiling residuals the absolute climb after the first few hundred evaluations is small.
...
\begin{table}[t]
  \centering
  \caption{Top-5 distinct fitted architectures on the CUDA inference bench (batch 64).
  Needs $R_{\mathrm{blend}}$ regeneration (still ranked under superseded prolonged $R$; not a current result).
  Favorite historically latency-best inside the near-max-$R$ band ($R\ge R_{\max}-5\cdot10^{-4}$).}
  \label{tab:top5}
...
Table~\ref{tab:sota-main} reports mean$\pm$std over the 20-waveform multi-family set (Section~\ref{sec:dataset}) from the \href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v11/figures/sota_matrix.json}{SOTA matrix}.
...
An endpoint-pin control (\href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v11/figures/jump_control.json}{pinning results}) drives wrap-jump to $0$ on all / hard subsets while dropping $R_{\mathrm{blend}}$ (all $0.733$, hard $0.597$).
...
\paragraph{Scores.}
Table~\ref{tab:va-seam-blep} reports batch means from the \href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v11/figures/va_seam_blep.json}{VA results} (seed \texttt{20260719}, batch 64) under $R_{\mathrm{blend}}$.
...
BLIT/BLEP nearly zeros wrap-jump ($0.004$) while dropping mean $R_{\mathrm{blend}}$ to $0.632$ and collapsing on hard cliffs ($0.351$).
BLAMP is near-no-bake on this value-cliff geometry (mean $0.914$).
...
They beat DualCosine on mean $R_{\mathrm{blend}}$ in places.
None matches the locked hybrid favorite on holdout $R_{\mathrm{blend}}$.
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}lrrrrrrr@{}}
    ...
    \bottomrule
  \end{tabular}\\[0.3em]
  {\footnotesize Latency (canonical, CUDA): PolyBLEP $\approx 0.87$\,ms/batch, DualCosine $\approx 1.46$, BLIT/BLEP $\approx 71$, BLAMP $\approx 20$. Frozen in the \href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v11/figures/va_seam_blep.json}{VA results}.}
\end{table*}
...
\begin{table}[t]
  \centering
  \caption{Rust \texttt{sound\_bench} transfer ($n{=}20$).
  Needs $R_{\mathrm{blend}}$ regeneration (table still shows superseded prolonged $R$).
  $\Delta R$ vs Dual Cosine.}
