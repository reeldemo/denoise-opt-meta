...
Figure~\ref{fig:dataset-distributions} summarizes the $4{,}096$-cycle metrics draw: ideal RMS clusters near $0.72$, wrap jumps span nearly three orders of magnitude (median $\approx 0.83$), and identity $R$ is high on easy tiles but falls below $0.91$ on the hardest decile of wrap jumps.
...
\begin{figure}[t]
  ...
  \caption{Evaluation corpus statistics on the frozen holdout seed (\texttt{20260719}, $n{=}4{,}096$ cycles).
    ...
    Bottom right: identity prolonged $R$ (no bake).
    Dashed and dotted vertical lines mark mean and median.
    Hard-cliff reporting (Section~\ref{sec:results}) uses the top 10\% wrap-jump stratum where identity $R$ drops.}
