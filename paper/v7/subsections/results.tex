...
On a pure value cliff the slope jump is small, so BLAMP is nearly an identity map.
...
PolyBLEP reaches mean $R\approx 0.929$ ($\Delta R\approx{+}0.104$ vs DualCosine) but trails identity / \texttt{seam\_fir3} / the meta favorite.
BLIT/BLEP nearly zeros wrap-jump ($0.004$) while dropping mean $R$ to $0.868$ and collapsing on hard cliffs ($R\approx 0.712$).
BLAMP is near-identity on this value-cliff geometry (mean $R\approx 0.972$).
