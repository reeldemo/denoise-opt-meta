...
No-bake (passthrough) scores $R\approx 0.972$ and is a ceiling only.
...
\begin{table}[t]
  \centering
  \caption{Method scores on the canonical sine{+}cliff holdout (batch 64, seed \texttt{20260719}, $L{=}256$, $N{=}16$). Mean$\pm$std over five consecutive seeds. No-bake is a passthrough ceiling. Primary metric is prolonged $R$ (1~=~best).}
...
RQ: when does hybrid search win vs no-bake / DualCosine / supervised nets?
...
No-bake/FIR wins on mild seams and OOD factory geometry (honest domain shift, not buried).
...
RQ: under a matched $5{,}000$-evaluation budget with LSTM in the searchable bake vocabulary, how do random search, CMA-ES, REINFORCE, aging evolution, and TPE compare to the hybrid (+LSTM) arm?
Table~\ref{tab:meta-approaches} reports champion prolonged $R$, $\Delta R$ vs DualCosine, wall hours, and whether the champion graph used an LSTM block.
...
\begin{figure}[t]
  ...
  \caption{Matched $5$k-budget meta-learning comparison: champion residual $R$ versus outer iteration (x-axis capped at $5{,}000$), including hybrid{+}LSTM. Accessibility: Okabe-Ito colors with distinct markers for grayscale print; DualCosine as a dashed gray baseline.}
...
Honest transfer note: the favorite was searched on Python \texttt{make\_batch} sine{+}cliff geometry. On Rust families it still beats DualCosine (mean $\Delta R{+}0.063$, Wilcoxon $p{\approx}0.009$) but trails no-bake and \texttt{seam\_fir3}. The primary SOTA claim remains the Python multi-family matrix.
