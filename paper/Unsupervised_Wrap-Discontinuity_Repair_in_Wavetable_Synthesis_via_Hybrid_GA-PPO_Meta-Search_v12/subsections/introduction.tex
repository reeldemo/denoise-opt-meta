...
\begin{itemize}
  \item \textbf{Seam} (wrap join / loop join): the junction between a period's end and start, where a click signals endpoint mismatch. The seam is local. The signal is the whole waveform.
  \item \textbf{Repair operator} ($\Theta$, short: operator): a searchable local edit applied at the seam (legacy name: bake cell).
  \item \textbf{Residual operator}: an optional tiny learned network inside a repair operator.
  \item \textbf{FitCell}: continuous-parameter fit of one candidate $\Theta$ under residual loss (Appendix~\ref{app:algorithms}).
  \item \textbf{$R$} / \textbf{$R_{\mathrm{blend}}$}: unsupervised seam-quality scores. Whole-curve prolonged $R$ is for debug. $R_{\mathrm{blend}}$ drives primary selection (Section~\ref{sec:residual-score}).
  \item \textbf{$J$}: latency-aware search objective $J=R_{\mathrm{blend}}-\lambda\cdot\mathrm{latency\_norm}$ (Section~\ref{sec:residual-score}).
  \item \textbf{MoE}: mixture-of-experts soft gating over small bake experts inside a repair operator.
  \item \textbf{PBT}: population-based training (exploit--mutate on elites in the outer loop).
  \item \textbf{Outer loop} / \textbf{meta-search}: genetic algorithm (GA) plus proximal policy optimization (PPO) search over repair-operator graphs, not end-to-end training of one large network.
  \item \textbf{Holdout}: frozen evaluation set or seed used for reported scores.
\end{itemize}
...
Under the locked v10.1 $R_{\mathrm{blend}}$ protocol (primary story: Table~\ref{tab:n2n-vs-ours}):
\begin{itemize}
  ...
  \item On twenty multi-family boards, Ours edges Noise2Noise on mean score ($0.931$ vs $0.925$, with $13/20$ wins). Both beat Dual Cosine.
...
\end{itemize}
In summary, compact searchable repair operators remain competitive when classical fades fail and need no paired clean targets at search time.
All primary claims use the locked v10.1 $R_{\mathrm{blend}}$ protocol (Section~\ref{sec:residual-score}).
Whole-curve prolonged $R$ near $0.991$ (including the historical freeze $R{\approx}0.99093$ vs Dual Cosine ${\approx}0.81658$) is a superseded debug score.
It is not a current holdout result under $R_{\mathrm{blend}}$.
...
They are mechanistic stress tests of wrap geometry under that protocol.
They are \emph{not} evidence of general domain diagnosis, and they are not claims that an audio-trained bake ``transfers'' as bearing or ECG SOTA.
...
We do not claim wrap-closure proofs, search-convergence theorems, or speech/music enhancement SOTA.
Caveats live in Section~\ref{sec:limitations}.
...
\begin{enumerate}
  ...
  \item \textbf{Applied hybrid search.} DenoiseOpt applies GA--PPO(+PBT) meta-search to compact repair-operator graphs for this DSP task (integrative engineering, not a new learning algorithm), scored by $R_{\mathrm{blend}}$ and ranked by $J$.
  \item \textbf{Open evaluation path.} Frozen Noise2Noise gate, multi-family and wavetable-native boards, and named wrap-protocol stress-test benches with released code and artifacts.
\end{enumerate}
