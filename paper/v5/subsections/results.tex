...
Optional edge RMSE (seam indices only) drops from identity $0.095$ to favorite $0.021$ on that hard subset.
...
RQ: does the wrap protocol transfer beyond synthetic sine cliffs?
Table~\ref{tab:real-wt} scores ReelSynth-style factory periods (primary realism, $n{=}24$) and OA additive-harmonic instrument cycles (secondary, $n{=}20$) after close-seam idealization and the same open-wrap cliff.
On factory periods the favorite ($R\approx 0.954$) trails identity / \texttt{seam\_fir3} slightly and beats DualCosine.
On OA instrument cycles the favorite ($R\approx 0.988$) leads DualCosine ($0.944$) and identity ($0.962$).
LibriSpeech/MUSDB are not used.

\begin{table}[t]
  \centering
  \caption{Wavetable-native wrap protocol. Close-seam ideal $\to$ open-wrap cliff (seed \texttt{20260719}). Mean prolonged $R$.}
  \label{tab:real-wt}
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{@{}lrr@{}}
    \toprule
    Method & Factory ($n{=}24$) & OA instrument ($n{=}20$) \\
    \midrule
    seq LSTM & 0.778 & 0.991 \\
    N2N sibling-sup. & 0.866 & 0.990 \\
    N2N corrupt$\to$corrupt & 0.838 & 0.990 \\
    neural favorite & 0.954 & 0.988 \\
    \texttt{seam\_fir3} & 0.962 & 0.968 \\
    identity & 0.959 & 0.962 \\
    DualCosine & 0.943 & 0.944 \\
    \bottomrule
  \end{tabular}
\end{table}
