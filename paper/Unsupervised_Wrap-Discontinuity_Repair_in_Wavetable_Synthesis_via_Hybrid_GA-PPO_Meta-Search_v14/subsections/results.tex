...
\paragraph{Abstract numbers $\mapsto$ tables.}
Every numeric claim in the abstract maps to a table here or in the appendix:
\begin{itemize}
  \item Holdout $0.9708{\pm}0.0014$ / N2N $0.9767{\pm}0.0015$ / Dual Cosine $0.542{\pm}0.038$ $\to$ Table~\ref{tab:n2n-vs-ours} (also Table~\ref{tab:param-fairness}).
  \item Matched-$5$k hybrid $0.9748{\pm}0.0018$ $\to$ Table~\ref{tab:meta-approaches}.
  \item Converge $R_{\mathrm{blend}}{\approx}0.9912$ (seed \texttt{1902771841}) $\to$ Table~\ref{tab:v14-hybrid-converge}.
  \item Multi-family sign-test $p{=}1.0$ $\to$ Section~\ref{sec:n2n-vs-ours-boards} (board-mean deltas).
\end{itemize}

\paragraph{Reading guide (artifact $\to$ finding).}
\begin{itemize}
  \item Holdout / N2N gate (Table~\ref{tab:n2n-vs-ours}, artifacts under \texttt{brand/artifacts/holdout\_*}): short-FitCell hybrid trails N2N, beats Dual Cosine (five eval seeds).
  \item Matched outer-loop bake-off (Table~\ref{tab:meta-approaches}, \texttt{meta\_approach\_compare\_v13\_rblend/}): hybrid leads six families (three search seeds) $\Rightarrow$ drop losers.
  \item FitCell-to-convergence (Table~\ref{tab:v14-hybrid-converge}, \texttt{meta\_approach\_compare\_v14\_converge/}): single completed seed clears N2N gate ($R_{\mathrm{blend}}{\approx}0.9912$).
  \item Hard cliffs / multi-family / Factory+AKWF: classical collapse vs residual hold; N2N still wins some native boards.
  \item Transfer (Table~\ref{tab:transfer-main}, \texttt{signal\_heal\_transfer/}): wrap-protocol stress test only.
  \item Ablations / HP (Appendix~\ref{app:supplement}, Tables~\ref{tab:ablation},~\ref{tab:hp-sensitivity}): short-budget GA/PPO drop checks and $\pm 50\%$ one-at-a-time HP probe---not matched-$5$k competitors.
\end{itemize}
Primary numbers use locked $R_{\mathrm{blend}}$ (Sections~\ref{sec:v10-freeze}--\ref{sec:n2n-vs-ours-boards}).
Matched bake-offs, HP probes, branch freezes, and transfer latency sit in Appendix~\ref{app:supplement}.
Honesty notes are collected in Section~\ref{sec:limitations}.
Reproduce with \texttt{REPRO.md} and \texttt{EVAL\_PROTOCOL.md} in this paper folder.

\subsection{Ablation definition}
\label{sec:ablation-def}
An \textbf{ablation} here means a controlled disable of one outer-loop ingredient while keeping FitCell and the operator vocabulary fixed.
\begin{itemize}
  \item \textbf{Type A (history tags).} Best prolonged-$R$ freeze labeled by branch (GA / PPO / PBT / \ldots) inside one recorded hybrid run (Table~\ref{tab:ablation}, left). Not a separate experiment.
  \item \textbf{Type B (restart).} Fresh $150$-iteration search with GA-only, PPO-only, GA+PPO, or full hybrid enabled; final cell re-scored under locked $R_{\mathrm{blend}}$ (Table~\ref{tab:ablation}, right; seed \texttt{1902771841}).
\end{itemize}
Neither type replaces the matched-$5$k six-family bake-off (Table~\ref{tab:meta-approaches}).
HP sensitivity is a separate one-at-a-time $\pm 50\%$ probe (Table~\ref{tab:hp-sensitivity}).

\subsection{Ablations and hyperparameter sensitivity (findings)}
\label{sec:ablation-hp-main}
\textbf{Finding (Type B).} Dropping GA or PPO does not collapse short-budget $R_{\mathrm{blend}}$ on the $150$-iteration probe relative to full hybrid on that seed (Table~\ref{tab:ablation}, right), but those scores stay far below the matched-$5$k hybrid mean.
\textbf{Finding (HP).} $\pm 50\%$ one-at-a-time shifts of population / clip / LR / mutate / entropy at $500$ iterations change champion $R$ by at most ${\approx}0.012$ (Table~\ref{tab:hp-sensitivity}); defaults are not claimed globally optimal.
...
Across five holdout seeds (\texttt{20260719}--\texttt{20260723}), frozen corrupt$\to$corrupt Noise2Noise---treated as the primary neural SOTA gate for this protocol, not a DenoiseOpt variant---leads the short-FitCell hybrid freeze on every seed
($0.9767{\pm}0.0015$ vs $0.9708{\pm}0.0014$; Dual Cosine $0.542{\pm}0.038$).
On the $20$-waveform multi-family board, Ours mean-of-means $0.9337{\pm}0.0028$ is numerically close to N2N $0.9308{\pm}0.0045$
(mean win fraction $0.47$ across seeds; sign test on board-mean deltas $p{=}1.0$, i.e.\ \emph{no} significant mean difference), and both dominate Dual Cosine.
We do \emph{not} claim a statistically significant win over N2N on that board.
...
\begin{table}[t]
  \centering
  \caption{Parameter-count and inference-latency fairness under the locked holdout story.
    ...
    Latency is CUDA ms/batch at $B{=}64$ (warmup 8, timed 30; Table~\ref{tab:ours-n2n-latency}).}
...
\noindent
Table~\ref{tab:param-fairness} states the size and latency comparison the review requested.
The short-FitCell hybrid favorite is larger and slower than SeamN2N on this freeze (${\approx}7.4\times$ holdout ms/batch).
That gap is partly by design: selection maximizes $J=R_{\mathrm{blend}}-\lambda\cdot\mathrm{latency\_norm}$, so search may accept a slower cell when residual gains outweigh the $\lambda$ penalty; it is not claimed as architectural superiority over N2N.
N2N still leads holdout $R_{\mathrm{blend}}$ on all five seeds under the short FitCell freeze.
Search seeds, N2N train seed \texttt{424242}, and holdout seeds stay disjoint.

\paragraph{Multi-seed protocol.}
Primary holdout and multi-family rows in Table~\ref{tab:n2n-vs-ours} report mean$\pm$std over five evaluation seeds.
Matched-$5$k outer-loop re-search under $R_{\mathrm{blend}}{+}J$ across three search seeds is complete
(\texttt{meta\_approach\_compare\_v13\_rblend/}; Table~\ref{tab:meta-approaches}:
Ours $0.9748{\pm}0.0018$, Aging $0.9635{\pm}0.0049$).

\subsection{Hybrid-only FitCell-to-convergence follow-up}
\label{sec:v14-converge}
Deferred~D1 (Table~\ref{tab:meta-approaches}) already ranked hybrid GA--PPO first among six outer-loop families under a short FitCell budget (${\approx}16$--$48$ steps).
We therefore stop spending matched budget on Random/CMA-ES/TPE/Aging/REINFORCE and reinvest the GPU in a \emph{hybrid-only} campaign where every candidate trains until residual plateau (or a searchable hard cap $T_{\max}\!\in\!\{512,\ldots,2048\}$), with searchable patience, relative $\varepsilon$, FitCell batch, and latency weight $\lambda$ (artifacts \texttt{meta\_approach\_compare\_v14\_converge/}; Table~\ref{tab:v14-hybrid-converge}).
Outer budget is $750$ evaluations with two sequential proposals per iteration (one CUDA process; no dual-seed VRAM duplication).
On search seed \texttt{1902771841}, the champion reaches $R_{\mathrm{blend}}{\approx}0.9912$, $J{\approx}0.9854$, $t{\approx}8.70$\,ms and \emph{clears} the frozen Noise2Noise gate (${\approx}0.9750$) after ${\approx}46.9$\,h wall.
The claim here is the D1 selection rationale plus this full hybrid-only converge run under locked $R_{\mathrm{blend}}{+}J$.

\begin{table}[t]
  \centering
  \caption{Hybrid-only FitCell-to-convergence follow-up after D1 (locked $R_{\mathrm{blend}}{+}J$).
    Gate: frozen Noise2Noise corrupt$\to$corrupt $R_{\mathrm{blend}}{\approx}0.9750$.
    Full $750$-evaluation run on search seed \texttt{1902771841}.}
  \label{tab:v14-hybrid-converge}
  \setlength{\tabcolsep}{3pt}
  \footnotesize
  \begin{tabular}{@{}lrrrc@{}}
    \toprule
    Search seed & Iters & $R_{\mathrm{blend}}$ & $t$\,ms & vs N2N gate \\
    \midrule
    \texttt{1902771841} & $750/750$ & $0.9912$ & $8.70$ & clears \\
    \bottomrule
  \end{tabular}
\end{table}
...
Table~\ref{tab:ours-n2n-latency} compares forward-pass wall time on CUDA across boards and batch sizes.
At the paper-default batch $B{=}64$, Ours is ${\approx}7\times$ slower than N2N on every board
(holdout $8.20$ vs $1.12$\,ms; multi-family mean $7.50{\pm}0.56$ vs $1.06{\pm}0.07$\,ms;
Factory+FX $7.31$ vs $1.06$\,ms; AKWF $7.50$ vs $1.11$\,ms).
The holdout batch sweep ($B\in\{1,8,64,256\}$) keeps the ratio in ${\approx}5$--$7\times$:
larger batches amortize per-sample cost for both methods, but N2N remains faster.
This matches the parameter gap (${\approx}121$k vs ${\approx}53.5$k) and the $J$ latency penalty in~\eqref{eq:outer-obj}.
...
The right panel is the locked $R_{\mathrm{blend}}$ / $J$ hybrid run over $1{,}200$ evaluations (seed \texttt{1902771841}):
raw champion $R_{\mathrm{blend}}$ clears Dual Cosine immediately and plateaus below the Noise2Noise gate (${\approx}0.977$).
Matched-$5$k multi-seed re-search under $R_{\mathrm{blend}}$ is complete
(Figure~\ref{fig:meta-approach-compare}; Table~\ref{tab:meta-approaches}).
...
Champion parameter count is not fixed: hybrid live champs are ${\approx}24$k$\pm$2k params across seeds,
while mid-search updates swing from ${\approx}15$k to ${\approx}370$k.
...
\begin{figure*}[t]
  ...
  \caption{Search learning curves (seed \texttt{1902771841}).
    ...
    (Dual Cosine and Noise2Noise gate marked; matched-$5$k multi-seed $R_{\mathrm{blend}}$ bake-off in Appendix~\ref{app:supplement}).}
