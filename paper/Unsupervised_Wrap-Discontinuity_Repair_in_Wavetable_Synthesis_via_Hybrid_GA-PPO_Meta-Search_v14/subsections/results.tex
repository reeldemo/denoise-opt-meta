...
Across five holdout seeds (\texttt{20260719}--\texttt{20260723}), Noise2Noise leads Ours on every seed
...
The hybrid favorite is larger and slower than SeamN2N on this freeze (${\approx}7.4\times$ holdout ms/batch).
N2N still leads holdout $R_{\mathrm{blend}}$ on all five seeds.
...
Outer budget is $750$ evaluations per seed with two sequential proposals per iteration (one CUDA process; no dual-seed VRAM duplication).
On completed seed \texttt{1902771841}, the champion reaches $R_{\mathrm{blend}}{\approx}0.9912$, $J{\approx}0.9854$, $t{\approx}8.70$\,ms and \emph{clears} the frozen Noise2Noise gate (${\approx}0.9750$) after ${\approx}46.9$\,h wall.
Seed \texttt{2026072701} is paused mid-run at $170/750$ with live champ $R_{\mathrm{blend}}{\approx}0.9895$ (also above the gate); seed \texttt{2026072702} is not started.
We do \emph{not} yet report a three-seed mean$\pm$std for this converge protocol; the claim here is the selection rationale from D1 plus the completed-seed result under honest pause status.
...
\begin{table}[t]
  ...
  \caption{Hybrid-only FitCell-to-convergence follow-up after D1 (locked $R_{\mathrm{blend}}{+}J$).
    ...
    Seed~\texttt{2026072701} is an interim checkpoint (search paused); seed~\texttt{2026072702} pending.}
  ...
  \begin{tabular}{@{}lrrrc@{}}
    ...
    \texttt{2026072701} & $170/750$ (paused) & $0.9895$ & $5.67$ & clears \\
    \texttt{2026072702} & $0/750$ & --- & --- & pending \\
    \bottomrule
  \end{tabular}
\end{table}
