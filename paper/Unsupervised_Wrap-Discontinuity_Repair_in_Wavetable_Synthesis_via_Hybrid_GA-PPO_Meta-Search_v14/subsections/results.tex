...
\subsection{Ablations and hyperparameter sensitivity (pointer)}
\label{sec:ablation-hp-main}
Branch ablations and HP sensitivity are reported in Appendix~\ref{app:supplement} so the main results stay on holdout / D1 / converge.
\textbf{Finding (ablation).} Under a $150$-iteration restart (seed \texttt{1902771841}), dropping GA or PPO does not collapse short-budget $R_{\mathrm{blend}}$ relative to full hybrid on that probe (Table~\ref{tab:ablation}, right); those runs are far below the matched-$5$k hybrid mean and must not overwrite Table~\ref{tab:meta-approaches}.
\textbf{Finding (HP).} One-at-a-time $\pm 50\%$ shifts of population / clip / LR / mutate / entropy at $500$ iterations change champion $R$ by at most ${\approx}0.012$ (Table~\ref{tab:hp-sensitivity}, Figure~\ref{fig:hp-sensitivity}); defaults are not claimed globally optimal.
