...
\begin{enumerate}
  ...
  \item \textbf{Longer FitCell} (Table~\ref{tab:v14-hybrid-converge}):
    one completed hybrid seed then clears the Noise2Noise residual gate.
...
The follow-up is hybrid-only: every candidate trains until residual plateau
...
Table~\ref{tab:v14-hybrid-converge} reports that completed run only.

\begin{table}[t]
  \centering
  \caption{Hybrid-only FitCell-to-convergence after D1 (locked $R_{\mathrm{blend}}{+}J$).
...
The left panel is a matched $5{,}000$-evaluation hybrid vs Random NAS under the older whole-curve prolonged $R$ metric
(seed \texttt{1902771841}; Dual Cosine ${\approx}0.817$).
Both arms clear Dual Cosine early; treat this panel as a search-trace artifact, not a locked-protocol claim.
The right panel is the locked $R_{\mathrm{blend}}$/$J$ hybrid over $1{,}200$ evaluations:
raw $R_{\mathrm{blend}}$ clears Dual Cosine immediately and plateaus below the Noise2Noise gate (${\approx}0.977$).
The matched multi-seed $R_{\mathrm{blend}}$ bake-off is in Table~\ref{tab:meta-approaches}.
Near the ceiling, scores climb only a little after the first few hundred evaluations.
Live hybrid champions stay compact (${\approx}24$k$\pm$2k params across seeds), while mid-search graphs can swing from ${\approx}15$k to ${\approx}370$k.
