...
Evaluation follows this paper folder's
\href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wrap-Discontinuity_Repair_in_Wavetable_Synthesis_via_Hybrid_GA-PPO_Meta-Search_v14/EVAL_PROTOCOL.md}{\texttt{EVAL\_PROTOCOL.md}}
(companion code: \url{https://github.com/reeldemo/reelsynth}).
\textbf{Locked $R_{\mathrm{blend}}$} means the frozen definition in Section~\ref{sec:residual-score}:
tile each cycle $N{=}16$ times, compute seam/body relative-RMS components~\eqref{eq:R-component} with wrap mask width $W{=}8$, then
$R_{\mathrm{blend}}=\alpha R_{\mathrm{seam}}+(1-\alpha)R_{\mathrm{body}}$ with $\alpha{=}0.7$~\eqref{eq:R-blend}.
No per-table rescaling: reported values are that clamp in $[0,1]$.
Search objective is $J$~\eqref{eq:outer-obj} ($\lambda{=}0.02$).
The champ gate is to strictly beat frozen Noise2Noise corrupt$\to$corrupt holdout $R_{\mathrm{blend}}$.
Secondary keys cover SNR, SDR, wrap-jump, and seam-local edge RMSE; whole-curve prolonged $R$ is debug-only.
...
The FitCell-to-convergence follow-up (Section~\ref{sec:v14-converge}) is a \emph{single completed} search seed (\texttt{1902771841}), not a three-seed mean.
...
PESQ/STOI stay out of scope on non-speech cycles, and LibriSpeech / MUSDB are not used.
...
\paragraph{Experimental setup (datasets, compute, reproducibility).}
\begin{itemize}
  ...
  \item \textbf{FitCell / outer loop.} Adam on $1-R_{\mathrm{blend}}$; D1 short cap ${\approx}16$--$48$ steps; converge campaign searchable patience/$\varepsilon$/$T_{\max}\!\in\!\{512,\ldots,2048\}$, batch ${\approx}96$, two sequential proposals/iter (Section~\ref{sec:pipeline-overview}).
    ...
    Hybrid-only converge: $750$ outer iters, seed \texttt{1902771841} completed (${\approx}46.9$\,h wall).
  ...
  \item \textbf{Commands / artifacts.} \texttt{REPRO.md}, \texttt{EVAL\_PROTOCOL.md}; frozen JSON under \texttt{brand/artifacts/} in \url{https://github.com/reeldemo/reelsynth}.
\end{itemize}
