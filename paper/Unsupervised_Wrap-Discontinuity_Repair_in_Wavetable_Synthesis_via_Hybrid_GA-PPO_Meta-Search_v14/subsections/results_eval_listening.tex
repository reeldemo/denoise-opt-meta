...
\paragraph{Listening statistics (honest).}
No formal MOS, MUSHRA, or forced-choice A/B study was run ($N_{\mathrm{listeners}}{=}0$).
The released pack has $n{=}5$ tiles $\times$ $3$ methods $=15$ WAVs for informal inspection only.
We therefore report \emph{no} listening mean$\pm$std, preference rates, or significance tests, and we do not invent them.
Objective companions remain cycle-local / prolonged $R$ on the same tiles and the vibrato spectrogram residual in Section~\ref{sec:vibrato-eval}.
Readers can download and listen. We do \emph{not} claim perceptual superiority from these files.
