...
This is an outer-loop bake-off under historical whole-curve prolonged $R$.
Noise2Noise is not an outer-loop method here (see Tables~\ref{tab:n2n-vs-ours} and~\ref{tab:sota-main}).
At the completed gate, Ours leads on absolute champion $R$ ($R{\approx}0.99146$, $\Delta R{\approx}{+}0.17488$ vs Dual Cosine ${\approx}0.81658$; ${\approx}8.46$\,h), with Aging evolution second ($R{\approx}0.99079$).
...
\begin{figure}[t]
  ...
  \caption{Matched 5,000-evaluation search methods (seed \texttt{1902771841}). Champion prolonged $R$; dashed line is Dual Cosine.}
...
\begin{figure}[t]
  ...
  \caption{Champion residual $R$ over a matched 5,000-evaluation budget. Dashed line: Dual Cosine.}
...
\begin{table}[t]
  \centering
  \caption{Ablations. Left: best score by branch in the recorded search run (final iteration ${\approx}6{,}922$). Right: isolated 150-iteration re-runs (seed \texttt{1902771841}). Prolonged $R$.}
  \label{tab:ablation}
...
Default champion $R{\approx}0.9888$; largest $|\Delta R|$ among completed arms ${\approx}0.0123$ (50\% lower learning rate).
...
N2N uses the wavetable checkpoint zero-shot; domain-trained N2N quality was not run.
