...
That outer agreement is scored by a discontinuity-weighted blend $R_{\mathrm{blend}}$ (seam fidelity vs ideal plus mid-cycle identity vs engine) under a latency-aware objective $J$ for this periodic seam artifact class.
...
We score cycle-local bake approximations of them beside DualCosine (Section~\ref{sec:va-seam-baselines}).
Open questions remain: which bake parameters, and which residual cell if any, minimize prolonged tiled error against a no-cliff ideal.
...
\paragraph{Primary baseline and blended residual.}
...
DualCosine remains a classical appendix row and a PPO advantage baseline, not the gate.
Search and champion selection maximize a latency-aware objective
...
(Section~\ref{sec:residual-score}; \texttt{EVAL\_PROTOCOL.md}).
...
\paragraph{Sci/eng transfer corpora (named).}
Beyond wavetable-native boards, we score the same wrap protocol on six cycle-local transfer sets
(Section~\ref{sec:transfer-protocol}):
\textbf{CWRU} bearings (Case Western Reserve University Bearing Data Center; drive-end @12\,kHz, shaft-rate windows with cubic ideal vs linear+cliff engine);
\textbf{MFPT} bearings (Society for Machinery Failure Prevention Technology Fault Data Sets; fixed 25\,Hz shaft, same cubic/linear construction, $n{=}256$ periods);
\textbf{MIT-BIH} Arrhythmia ECG (PhysioNet; R--R normal beats with local-mean template ideal);
\textbf{PTB-XL} ECG 500\,Hz subset (PhysioNet \texttt{records500}/\texttt{*\_hr} lead-I beat windows; subset pilot, not the full corpus);
\textbf{synth CNC G01} (procedural closed-contour residual with sharp corners vs wide blend; KIT CNC DOI login-walled, so synthetic only);
\textbf{synth PMU/AC cycle} (procedural harmonic voltage cycle with wrap cliff; IEEE DataPort PMU OA account-walled, so synthetic only).
All transfer tables are classical-board comparisons unless a deep row is explicitly executed.

This paper contributes a meta-learning protocol for this periodic seam instance.
