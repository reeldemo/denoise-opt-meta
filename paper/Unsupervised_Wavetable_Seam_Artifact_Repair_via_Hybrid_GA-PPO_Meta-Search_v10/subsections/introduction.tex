...
Search and champion selection maximize latency-aware $J$~\eqref{eq:outer-obj}
($\lambda{=}0.02$), where
...
As a cross-domain stress test (not a diagnosis claim), we reuse the same wrap protocol on six cycle-local boards
...
On each board we train/search the hybrid bake cell under prolonged residual $R$
(historical transfer freeze; not the v10.1 $R_{\mathrm{blend}}$ lock)
and compare against classical baselines; domain-trained Noise2Noise quality was not run.
