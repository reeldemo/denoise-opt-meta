...
Under the blended residual and latency objective, does hybrid GA--PPO beat the frozen Noise2Noise gate?
...
Artifacts: \href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/meta_approach_compare_v10.json}{v10 search data}.
...
\paragraph{Earlier whole-curve scores (context only).}
Earlier whole-curve prolonged-$R$ freezes ($R{\approx}0.991$ vs DualCosine ${\approx}0.825$) are historical context only;
...
\paragraph{Earlier whole-curve search freeze.}
...
(\href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/inference_bench.json}{inference data}, batch 64), preferring residual $R$ first and latency on ties,
...
Does the meta favorite beat the classical non-AI board (no-bake, DualCosine, FIR, poly, fades) and fixed MLP/CNN baselines across twenty generative families?
Table~\ref{tab:sota-main} reports mean$\pm$std over the 20-waveform multi-family set (Section~\ref{sec:dataset}), with canonical-holdout SNR/SDR/wrap-jump for the same method set from the \href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/sota_matrix.json}{SOTA matrix}.
...
\begin{table*}[t]
  \centering
  \caption{Comparison across 20 generative waveforms (10 families $\times$ 2 seeds, batch 64).
...
Table~\ref{tab:cliff-strata} stratifies a $4{,}096$-tile holdout draw (seed \texttt{20260719}) by engine wrap-jump percentiles (p75$\approx 1.387$, p90$\approx 1.832$; frozen in the \href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/cliff_strata.json}{cliff-strata data}).
...
\begin{table}[t]
  \centering
  \caption{Required seam-local secondary on top-10\% wrap-jump tiles ($n{=}410$): edge RMSE (lower better). Frozen in the \href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/cliff_strata.json}{cliff-strata data}.}
...
A seam-local degree-3 polynomial fitter (no learning) reaches canonical $R\approx 0.972$ ($\approx$ no-bake) with wrap-jump essentially unchanged (\href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/poly_baseline.json}{polynomial results}).
SSM seq models are deferred; the LSTM remains the supervised seq ceiling.
An endpoint-pin control (\href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/jump_control.json}{pinning results}) drives wrap-jump to $0$ on all / hard subsets while dropping $R$ (all $0.906$; hard $0.822$).
...
\paragraph{Scores.}
Table~\ref{tab:va-seam-blep} reports batch means from the \href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/va_seam_blep.json}{VA results} (seed \texttt{20260719}, batch 64).
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}lrrrrrrr@{}}
    ...
    \bottomrule
  \end{tabular}\\[0.3em]
  {\footnotesize Latency (canonical, CUDA): PolyBLEP $\approx 1.43$\,ms/batch; DualCosine $\approx 1.06$; BLIT/BLEP $\approx 123$; BLAMP $\approx 54$. Frozen in the \href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/va_seam_blep.json}{VA results}.}
\end{table*}

\subsection{When search helps vs classical methods}
\label{sec:transfer}

When does hybrid search win vs the classical non-AI board (no-bake / FIR / DualCosine) and supervised nets?
Per-cycle $\Delta R$ on export, AKWF, Rust \texttt{sound\_bench}, and sine{+}cliff holdout (\href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/transfer_failures.json}{transfer results}):
...
How do search branch-best freezes compare to short isolated GA/PPO budgets, and what compute was used?
...
Table~\ref{tab:compute} summarizes the search-campaign compute budget. Peak VRAM is from an \texttt{nvidia-smi} poll during a short full-branch search replay (30 iters) plus a champion forward{+}FitCell replay. The campaign log did not record memory.

\begin{table}[t]
  \centering
  \caption{Ablations. Left column: best score reached by each branch in the recorded search run (final iteration $\approx 6{,}922$). Right column: isolated 150-iteration re-runs (seed \texttt{1902771841}). $R$ is prolonged residual on the runner geometry.}
  \label{tab:ablation}
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{@{}lrr@{}}
    \toprule
...
\begin{table}[t]
  \centering
  \caption{Compute budget for the reported 5,000-evaluation search run (RTX~3090, PyTorch $2.6$+cu124, search seed \texttt{1902771841}). Architecture evaluations approximately equal clean iterations.}
  \label{tab:compute}
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{@{}lr@{}}
    \toprule
...
Under a matched $5{,}000$-evaluation budget with LSTM and xLSTM in the searchable bake vocabulary, how do Random NAS, Cont.\ CMA-ES, Arch REINFORCE, Aging evolution, and TPE Bayes NAS compare to \textbf{Ours (hybrid GA--PPO)}?
...
Source: \href{https://github.com/reeldemo/denoise-opt-meta/blob/master/paper/Unsupervised_Wavetable_Seam_Artifact_Repair_via_Hybrid_GA-PPO_Meta-Search_v10/figures/meta_approach_compare.json}{matched-budget results} (search seed \texttt{1902771841}).
...
\begin{figure}[t]
  ...
  \caption{Matched 5,000-evaluation comparison of search methods (search seed \texttt{1902771841}). Bars report champion prolonged $R$; the dashed line is the DualCosine reference. Colors and hatches remain distinguishable in grayscale.}
...
\begin{figure}[t]
  ...
  \caption{Champion residual $R$ over a matched 5,000-evaluation search budget. The dashed line is the DualCosine reference; the objective is absolute $R$ toward the ideal sibling. Colors and markers remain distinguishable in grayscale.}
...
Does the Python-trained favorite transfer to Rust \texttt{sound\_bench} tiles against the classical non-AI board?
...
\begin{figure}[t]
  ...
  \caption{Champion residual $R$ over the first 5,000 search iterations, with the DualCosine reference on the same runner geometry (RTX~3090). The black line uses circle markers; the reference is dashed gray.}
...
\begin{figure}[t]
  ...
  \caption{Best residual score reached by each search branch over the first 5,000 iterations (PPO/RL, GA, PBT, NAS, and combined). Colors and markers remain distinguishable in grayscale.}
...
\begin{figure*}[t]
  ...
  \caption{Search progress over the first 5,000 iterations: champion residual $R$ (top) and best residual by search branch (bottom). Markers and colors match Figure~\ref{fig:branch-bests}.}
...
\begin{figure}[t]
  ...
  \caption{Per-trial residual scores by search branch over the first 5,000 iterations, with the champion trajectory overlaid. Branch markers match Figure~\ref{fig:branch-bests}.}
...
\begin{figure}[t]
  ...
  \caption{Champion updates over the first 5,000 iterations. Sparse $R$ labels mark the first, middle, and final improvements for two-column legibility.}
