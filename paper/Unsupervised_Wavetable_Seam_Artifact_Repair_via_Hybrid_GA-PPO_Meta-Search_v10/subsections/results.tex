...
Search-monitor Dual Cosine on the same runner is $R_{\mathrm{blend}}{\approx}0.504$
($\Delta R_{\mathrm{blend}}\approx{+}0.466$ vs Dual Cosine).
...
even though it clearly clears Dual Cosine.
...
\begin{table}[t]
  \centering
  \caption{Ours vs Noise2Noise \emph{and} Dual Cosine under $R_{\mathrm{blend}}$ ($\alpha{=}0.7$).
    Holdout seed \texttt{20260719}. Multi-family: $10$ families $\times$ $2$ seeds.
    Factory+FX / AKWF: $n{=}1{,}280$ each.
    $\Delta_{\mathrm{N2N}}$ / $\Delta_{\mathrm{DC}}$ are Ours minus that baseline;
    Gate: whether Ours strictly exceeds N2N.}
...
Primary reading is absolute prolonged-$R$ rank:
...
This is an outer-loop bake-off under historical whole-curve prolonged $R$; Noise2Noise is not an outer-loop method here
(see Tables~\ref{tab:n2n-vs-ours} and~\ref{tab:sota-main} for Ours vs N2N vs Dual Cosine).
