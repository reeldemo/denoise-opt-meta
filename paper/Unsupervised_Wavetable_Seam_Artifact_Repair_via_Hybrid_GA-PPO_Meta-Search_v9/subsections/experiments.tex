...
Evaluation follows \texttt{EVAL\_PROTOCOL.md} in this paper folder (extended for peer-review strata).
...
Venue positioning: DSP / synthesis artifact repair (DAFx / AES / arXiv cs.SD).
This is not a general speech-enhancement leaderboard paper.
...
Figure~\ref{fig:dataset-families} counts cycles per family ($128$ each).
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_dataset_family_counts.png}
  \caption{Multi-family SOTA board: $128$ cycles per family ($2$ seeds $\times$ batch $64$).}
  \label{fig:dataset-families}
\end{figure}
...
Wavetable-native boards use ReelSynth factory exports ($n{=}25$) and AKWF CC0 cycles ($n{=}24$).
Sci/eng transfer tiles $1{,}408$ periods across CWRU, MFPT, MIT-BIH, PTB-XL, and synthetic CNC/PMU (cache metas under \texttt{signal\_heal\_transfer/cache/}).
Search never trains on the frozen holdout; N2N/seq baselines use disjoint train seeds \texttt{424242}/\texttt{424243}.
Artifacts: \texttt{dataset\_inventory.json}, \texttt{fig\_dataset\_*.png}.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}llp{0.42\columnwidth}@{}}
    ...
    Meta $5$k compare & $6$ approaches & shared seed; batch $48$ \\
    CWRU & $256$ periods & $4$ MAT files; DE @12\,kHz; ${\approx}0.025$\,h raw \\
    MFPT & $128$ periods & shaft-rate windows \\
    MIT-BIH & $256$ periods & $10$ records @360\,Hz; ${\approx}3.76$\,h raw \\
    PTB-XL & $256$ periods & $94$ \texttt{*\_hr} @500\,Hz; ${\approx}0.54$\,h raw \\
