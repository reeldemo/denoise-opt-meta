...
Matched-$5$k outer-loop re-search under $R_{\mathrm{blend}}$ across three search seeds is running into
\texttt{meta\_approach\_compare\_v13\_rblend/} (Section~\ref{sec:search-learning-curve}).
...
Matched-$5$k multi-seed re-search under $R_{\mathrm{blend}}$ (hybrid first, then Random and other arms) populates
\texttt{meta\_approach\_compare\_v13\_rblend/} for updated learning curves as seeds finish.
...
\begin{figure*}[t]
  ...
  \caption{Search learning curves (seed \texttt{1902771841}).
    ...
    (Dual Cosine and Noise2Noise gate marked; multi-seed $R_{\mathrm{blend}}$ re-search in progress).}
